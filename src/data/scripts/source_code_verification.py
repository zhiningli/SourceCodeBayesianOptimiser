...
source_codes_pending_validation = sourceCodeRepository.find_source_codes(status=source_code_status, limit = 10)
...
for source_code_object in source_codes_pending_validation:
    ...
    if validated_source_code:
        source_code_object['source_code'] = source_code_validator
        sourceCodeRepository.update_source_code(record_id=source_code_id, update_data= source_code_object, status=SourceCodeStatus.VALIDATED_TO_RUN.value)
    else:
