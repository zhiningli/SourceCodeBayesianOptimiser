...
source_codes_pending_validation = sourceCodeRepository.find_source_codes(status=source_code_status, limit = 200)
...
for source_code_object in source_codes_pending_validation:
    ...
    if validated_source_code:
        ...
        source_code_object['source_code'] = source_code_block
        source_code_object['timestamp'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
