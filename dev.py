...
from src.utils.benchmark_functions import Beale, Sphere
...
def dev_test():
    ...
    surrogate = GP(kernel=kernel, noise=1e-1)
    acquisition_func = EI(xi=0.02)
    optimiser = Optimiser(acquisition=acquisition_func, model=surrogate, n_iter=10, objective_func=benchmark.evaluate)

    # Generate initial points within the search space
    X_init = np.random.uniform(-5.12, 5.12, (15, 2))  # 5 initial points in a 2D space
    ...
    # Plot the Beale function's contour and evaluation points
    plot_sphere_function_3d(benchmark, all_points)

def plot_beale_function_3d(benchmark, evaluation_points):
    ...
    ax.scatter(3, 0.5, 0, color='white', marker='x', s=100, label='Global Minimum (3, 0.5)')

    # Labels and title
    ax.set_title('3D Beale Function with Evaluation Points')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ...
    plt.savefig('beale_function_3d_optimization.png')
    print("3D plot saved as 'beale_function_3d_optimization.png'")
